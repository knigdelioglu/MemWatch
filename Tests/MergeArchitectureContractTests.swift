...
struct MergeArchitectureContractTests {
    static func main() throws {
        ...
        expect(displayFeatureView.contains("@State private var brightnessDraft: Double = 0")
            ...
            && displayFeatureView.contains("@State private var brightnessTask: Task<Void, Never>?")
            && displayFeatureView.contains("get: { brightnessDraft }")
            && displayFeatureView.contains("ExternalSliderInteractionPolicy.brightnessDebounceNanoseconds")
            && displayFeatureView.contains("brightnessTask?.cancel()")
            && displayFeatureView.contains("guard !Task.isCancelled else { return }")
            && displayFeatureView.contains("display.setMonitorBrightness(intValue)")
            ...
            && displayFeatureView.contains("ExternalSliderInteractionPolicy.volumeDebounceNanoseconds")
        ...
        expect(unifiedSettingsView.contains("@State private var brightnessDraft: Double = 0")
            ...
            && unifiedSettingsView.contains("@State private var brightnessTask: Task<Void, Never>?")
            && unifiedSettingsView.contains("get: { brightnessDraft }")
            && unifiedSettingsView.contains("ExternalSliderInteractionPolicy.brightnessDebounceNanoseconds")
            && unifiedSettingsView.contains("brightnessTask?.cancel()")
            && unifiedSettingsView.contains("display.setMonitorBrightness(intValue)")
        ...
        expect(displayBrightnessRuntime.contains("manualBrightnessInteractionActive")
            ...
            && displayCoordinatorState.contains("startManualBrightnessWrite"),
               "Brightness completions must be latest-wins and manual interaction must suppress auto writes")
