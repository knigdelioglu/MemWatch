...
struct CleanupRuleCatalog: Sendable {
    ...
    static let defaultRules: [CleanupRule] = [
        ...
        CleanupRule(id: "xcode.simulatorcache", category: .xcode, rootPolicy: .userLibrary, defaultSafety: .review, deletionMode: .maintenance, requirements: [.explicitConfirmation, .applicationInactive], description: "Simulator cache or unavailable-runtime candidate; final cleanup must use the Xcode/simctl backend where appropriate"),
        ...
        CleanupRule(id: "application.leftover.user", category: .applicationLeftovers, rootPolicy: .userLibrary, defaultSafety: .review, deletionMode: .trash, requirements: [.explicitConfirmation], description: "High-confidence leftover belonging to an uninstalled application"),
        ...
        CleanupRule(id: "launchitem.orphan", category: .launchItems, rootPolicy: .systemLibrary, defaultSafety: .review, deletionMode: .privileged, requirements: [.privilegedHelper, .explicitConfirmation], description: "Launch item whose owning application and executable are both absent"),
