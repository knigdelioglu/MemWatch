...
actor CleanupScanEngine {
    ...
    init(
        scanners: [any CleanupScanner] = CleanupScannerRegistry.userSpaceScanners,
    ...
    func scan(
                    ...
                    guard candidate.scannerID == scanner.id else {
                        issues.append(
                            CleanupScanIssue(
                                ...
                                message: "Scanner returned a candidate with a mismatched scanner ID"
                            )
                        )
                        continue
                    }
                    ...
                    guard let rule = ruleCatalog.rule(for: candidate.ruleID) else {
                        issues.append(
                            CleanupScanIssue(
                                scannerID: scanner.id,
                                path: candidate.url.path,
                                message: "Unknown cleanup rule: \(candidate.ruleID.rawValue)"
                            )
                        )
                        continue
                    }

                    let assessment = safetyEngine.assess(candidate: candidate, rule: rule, context: context)
                    items.append(assessment.candidate)
                }
        ...
        let sortedItems = items.sorted { lhs, rhs in
            if lhs.safety != rhs.safety {
                return lhs.safety.rawValue < rhs.safety.rawValue
            }
