...
public final class HiDPIReapplyService {
    ...
    public func suspendForPowerTransition() {
        suspendForTargetTransition()
    }

    /// Target re-enumeration is a readiness boundary even when the global
    /// power state remains active. Preserve one pending preference retry while
    /// cancelling any old target-epoch work.
    public func suspendForTargetTransition() {
    ...
    /// The coordinator also observes AppKit screen-parameter notifications.
    /// Ignore the notification generated by our own CGS transaction so it
    /// cannot restart target readiness in the middle of a single-flight apply.
    public var shouldSuppressDisplayChangeRecovery: Bool {
        manualSwitchSuppressionDepth > 0 ||
            lifecycleState.isApplyingMode ||
            Date() < selfGeneratedCallbackSuppressionUntil
    }
    ...
    private func handleDisplayReconfigurationCallback() {
        ...
        guard powerSnapshot.isActive,
              powerSnapshot.isHiDPIAllowed,
              powerSnapshot.targetDisplayReadiness.isReady else {
    ...
    private var canRunHiDPIOperations: Bool {
        ...
        return snapshot.isActive &&
            snapshot.isHiDPIAllowed &&
            snapshot.targetDisplayReadiness.isReady
    }
    ...
    private func executeReapply(operationGeneration: UInt64) async {
        ...
        let startingPowerSnapshot = currentPowerSnapshot
        ...
        do {
            ...
            guard resolvedDisplay.isOnline,
                  ...
                  resolvedDisplay.displayID == startingPowerSnapshot.targetDisplayReadiness.displayID,
            ...
            guard canRunHiDPIOperations,
                  currentPowerSnapshot.targetDisplayGeneration == startingPowerSnapshot.targetDisplayGeneration,
                  currentPowerSnapshot.targetDisplayReadiness == startingPowerSnapshot.targetDisplayReadiness else { return }
